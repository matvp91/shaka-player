...
shaka.media.StreamingEngine = class {
  ...
  constructor(manifest, playerInterface) {
    ...
    this.crossBoundaryTimer_ = new shaka.util.Timer(() => {
      ...
      if (video.ended || video.readyState === 0) {
        // With no metadata (e.g. mid MediaSource reset), a seek would fight
        // the position restore. forwardTimeForCrossBoundary_ will re-arm us
        // once the element is ready again.
        return;
      }
      if (this.boundaryTime_ != null &&
        ...
        video.currentTime = this.boundaryTime_;
        // boundaryTime_ stays set while the crossing is in flight, so that
        // isCrossingBoundary() reports the transition until the playhead
        // has settled in the new boundary.  It is cleared by
        // forwardTimeForCrossBoundary_ on the next evaluation after the
        // transition.  The currentTime guard above makes re-fires no-ops.
      }
    });
  ...
  update_(mediaState) {
    ...
    if (mediaState.type != ContentType.TEXT &&
      ...
      // TODO: temporary investigation logging, remove before merging.
      shaka.log.info('resetMediaSource: media source closed');
  ...
  async handleStreamingError_(mediaState, error) {
    ...
    if (error.code == shaka.util.Error.Code.MEDIA_SOURCE_OPERATION_THREW ||
      ...
      if (error.code == shaka.util.Error.Code.MEDIA_SOURCE_OPERATION_THREW ||
        ...
        // TODO: temporary investigation logging, remove before merging.
        shaka.log.info('resetMediaSource: recovering from error',
            error.code);
  ...
  forwardTimeForCrossBoundary_() {
    ...
    if (video.readyState === 0 ||
        (this.boundaryTime_ != null && video.currentTime === 0)) {
      // Mid MediaSource reset (no metadata yet), or the reset finished but
      // the playback position has not been restored yet: there is no
      // meaningful position to measure against, and boundaryTime_ must
      // survive so that isCrossingBoundary() keeps reporting the in-flight
      // transition. We'll be re-invoked via seeked/timeupdate once the
      // playhead has settled.
      return;
    }

    const presentationTime = video.currentTime;
    ...
    if (!lastInitRef) {
      // Nothing has been appended since a MediaSource reset; keep
      // boundaryTime_ for the still in-flight transition.
      return;
    }

    // From here on the boundary state can be re-derived: boundaryTime_ is
    // non-null exactly while a crossing is imminent or in flight.
    this.boundaryTime_ = null;

    if (lastInitRef.boundaryEnd === null) {
      // Playing the last known boundary; there is nothing to cross.
      return;
    }

    const threshold = shaka.media.StreamingEngine.CROSS_BOUNDARY_END_THRESHOLD_;
    const fudge = shaka.media.StreamingEngine.APPEND_WINDOW_END_FUDGE_;
    const fromEnd = lastInitRef.boundaryEnd - presentationTime;
    // Check if within threshold to eliminate a backwards seek.  Buffered
    // media can legitimately extend up to boundaryEnd + fudge (the append
    // window fudge admits frames slightly past the period end), so the
    // playhead may stall marginally past boundaryEnd.  A slightly negative
    // fromEnd is still a forward crossing, not a backwards seek.
    if (fromEnd < -fudge || fromEnd > threshold) {
      return;
    }

    // Set the intended time to seek to in order to cross the boundary.
    this.boundaryTime_ = lastInitRef.boundaryEnd + fudge;
    ...
    this.crossBoundaryTimer_.tickAfter(Math.max(0, fromEnd));
  }

  /**
   * Returns whether a cross-boundary transition is imminent or in flight:
   * a boundary seek is scheduled or was issued and the playhead has not yet
   * settled in the new boundary.  During the transition's MediaSource reset
   * the playback position is transiently meaningless (it may read 0 until
   * the position restore lands), so observers of the playhead should not
   * reinterpret it.
   *
   * @return {boolean}
   */
  isCrossingBoundary() {
    return this.boundaryTime_ != null;
  }
  ...
  discardReferenceByBoundary_(mediaState, reference) {
    ...
    if (leadingMediaState && discard &&
      ...
      // TODO: temporary investigation logging, remove before merging.
      shaka.log.info('resetMediaSource: cross-boundary discard');
