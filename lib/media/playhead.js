...
shaka.media.MediaSourcePlayhead = class {
   ...
   * @param {(function():boolean)=} isCrossingBoundary
   *     Returns true while a cross-boundary transition is in flight.  During
   *     its MediaSource reset the playback position is transiently
   *     meaningless, so it must not be reinterpreted as having fallen out of
   *     the live window.
   */
  constructor(mediaElement, manifest, config, startTime, onSeek, onEvent,
      getPlaybackRate, isCrossingBoundary) {
    /**
    ...
    this.isCrossingBoundary_ = isCrossingBoundary || (() => false);
  ...
  onPollWindow_() {
    ...
    if (this.isCrossingBoundary_()) {
      // A cross-boundary MediaSource reset is in flight; the playback
      // position is transiently meaningless (it may read 0 until the
      // position restore lands).  Do not treat it as having fallen out of
      // the live window.
      return;
    }
