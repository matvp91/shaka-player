    ...
    if (!this.templateInfo_) {
      ...
      if (this.templateInfo_.unscaledPresentationTimeOffset !==
        ...
        for (let i = 0; i < info.timeline.length; i++) {
          ...
          // The unscaled values are PTO-subtracted, so adding the delta
          // rebases them into the cached PTO frame.  After that, the same
          // physical segment (same t in the manifest) has the same unscaled
          // times as before the PTO change, and merging below will not
          // duplicate it.
          ...
          range.start = range.unscaledStart / info.timescale;
          range.end = range.unscaledEnd / info.timescale;
        }
      }
