  ...
  appendTemplateInfo(info, periodStart, periodEnd, initSegmentReference,
    ...
    if (!this.templateInfo_) {
      ...
      if (this.templateInfo_.unscaledPresentationTimeOffset 
        ...
        const delta = info.unscaledPresentationTimeOffset - this.templateInfo_.unscaledPresentationTimeOffset;

        shaka.log.error('Suboptimal manifest detected!',
          'The presentationTimeOffset differs from the cached template info by',
          delta,
          '. Shifting timeline to mitigate.'
        );
        ...
        for (let i = 0; i < info.timeline.length; i++) {
          ...
          // TODO(matvp): What with segmentPosition. $Number$ lookup would fail.
        }
      }
